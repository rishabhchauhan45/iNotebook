...
const Notes = () => {
    ...
    useEffect(() => {
        if (localStorage.getItem('token')) {
            getNotes() // This calls the API to fetch notes
        }
        else {
    ...
    // Function to open the Modal with current note details
    const updateNote = (currentNote) => {
        ref.current.click();
        setNote({ id: currentNote._id, etitle: currentNote.title, edescription: currentNote.description, etag: currentNote.tag })
    }

    // Function to call the edit API when Update button is clicked
    const handleClick = (e) => {
